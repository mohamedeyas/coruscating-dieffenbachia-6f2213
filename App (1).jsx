...
function POSPage({ products, customers, sales, setSales }) {
  ...
  const receivedAmt = receivedInput === "" ? total : Math.min(total, Math.max(0, parseFloat(receivedInput) || 0));
  ...
  const handleCheckout = () => {
    ...
    const paidStatus = receivedAmt <= 0 ? "Unpaid" : receivedAmt >= total ? "Paid" : "Partial";
    const invoice = {
      id: invNum,
      date: today,
      customer: custName,
  ...
  if (success) return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 400, gap: 20 }}>
      ...
      <div style={{ background: "#fff", border: `1px solid ${BRAND.border}`, borderRadius: 12, padding: "16px 24px", minWidth: 220 }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, color: BRAND.gray, marginBottom: 6 }}>
          <span>Total</span><span style={{ fontWeight: 700, color: BRAND.dark }}>₹{lastInvoice.total.toFixed(2)}</span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, color: BRAND.gray, marginBottom: 6 }}>
          <span>Paid</span><span style={{ fontWeight: 700, color: BRAND.green }}>₹{lastInvoice.paid.toFixed(2)}</span>
        </div>
        {lastInvoice.total - lastInvoice.paid > 0.004 && (
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, borderTop: `1px dashed ${BRAND.border}`, paddingTop: 6 }}>
            <span style={{ color: "#dc2626", fontWeight: 700 }}>Balance Due</span>
            <span style={{ fontWeight: 800, color: "#dc2626" }}>₹{(lastInvoice.total - lastInvoice.paid).toFixed(2)}</span>
          </div>
        )}
      </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 20, height: "calc(100vh - 140px)" }}>
      ...
      <div style={{ background: "#fff", border: `1px solid ${BRAND.border}`, borderRadius: 20, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ borderTop: `1px solid ${BRAND.grayLight}`, padding: "12px 20px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ fontSize: 13, color: BRAND.gray }}>Received Amount</span>
            <input type="number" value={receivedInput} placeholder={total.toFixed(2)}
              onChange={e => setReceivedInput(e.target.value)}
              style={{ width: 100, padding: "4px 8px", border: `1.5px solid ${BRAND.border}`, borderRadius: 8, fontSize: 13, textAlign: "right", outline: "none" }} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, fontWeight: 700, marginBottom: 12, color: balance > 0.004 ? "#dc2626" : BRAND.green }}>
        ...
        <div style={{ padding: "16px 28px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}>
            <div style={{ minWidth: 260, fontSize: 13 }}>
              ...
              {discAmt > 0.004 && (
                <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #e5e7eb" }}>
                  <span style={{ color: "#6b7280" }}>Discount{inv.discount ? ` (${inv.discount}%)` : ""}</span><span style={{ fontWeight: 600, color: "#16a34a" }}>-₹{discAmt.toFixed(2)}</span>
                </div>
              )}
              ...
              {typeof inv.paid === "number" && (
                <>
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", marginTop: 8 }}>
                    <span style={{ color: "#6b7280" }}>Amount Paid</span><span style={{ fontWeight: 600, color: "#16a34a" }}>₹{inv.paid.toFixed(2)}</span>
                  </div>
                  {total - inv.paid > 0.004 && (
                    <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderTop: "1px dashed #d1d5db" }}>
                      <span style={{ fontWeight: 700, color: "#dc2626" }}>Balance Due</span><span style={{ fontWeight: 800, color: "#dc2626" }}>₹{(total - inv.paid).toFixed(2)}</span>
                    </div>
                  )}
                </>
              )}
...
function InvoicesPage({ sales, setSales }) {
  ...
  const deriveStatus = (total, paid) => (paid <= 0.004 ? "Unpaid" : paid >= total - 0.004 ? "Paid" : "Partial");

  const saveEdit = () => {
    if (!editForm) return;
    const total = +editForm.total;
    const paid = Math.max(0, Math.min(total, +editForm.paid));
      ...
      {sales.length === 0 ? (
        ...
        <div style={{ background: "#fff", border: `1px solid ${BRAND.border}`, borderRadius: 16, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: BRAND.grayLight }}>
                {["Invoice #", "Customer", "Date", "Items", "Total", "Paid", "Balance", "Status", "Actions"].map(h => (
                  <th key={h} style={{ textAlign: "left", padding: "12px 16px", fontSize: 12, color: BRAND.gray, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((s, i) => (
                <tr key={s.id} style={{ borderBottom: `1px solid ${BRAND.grayLight}`, background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                  ...
                  <td style={{ padding: "13px 16px", fontSize: 13, fontWeight: 600, color: (s.total - s.paid) > 0.004 ? "#dc2626" : BRAND.gray }}>₹{(s.total - s.paid).toLocaleString("en-IN")}</td>
      ...
      <Modal open={!!editInv} onClose={() => { setEditInv(null); setEditForm(null); }} title={`Edit Invoice ${editInv?.id || ""}`}>
        {editForm && (
          <>
            ...
            <Input label="Balance (₹)" type="number" value={(+editForm.total - +editForm.paid).toFixed(2)}
              onChange={e => setEditForm(f => ({ ...f, paid: (+f.total - (parseFloat(e.target.value) || 0)).toFixed(2) }))} />
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "8px 0 4px" }}>
              <span style={{ fontSize: 13, color: BRAND.gray, fontWeight: 600 }}>Status (auto)</span>
              <Badge status={deriveStatus(+editForm.total, +editForm.paid)} />
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
...
function SettingsPage({ logoUrl, setLogoUrl, settings, setSettings }) {
  ...
  const [apiBase, setApiBase] = useState(() => { try { return localStorage.getItem("billing_api_base") || ""; } catch { return ""; } });
  const [apiSaved, setApiSaved] = useState(false);
  const [apiStatus, setApiStatus] = useState(null); // null | "checking" | "ok" | "fail"

  const saveApiBase = () => {
    try { localStorage.setItem("billing_api_base", apiBase.trim().replace(/\/$/, "")); } catch {}
    setApiSaved(true);
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 20 }}>
      ...
      <div style={{ background: "#fff", border: `1px solid ${BRAND.border}`, borderRadius: 16, padding: 28 }}>
        {tab === "company" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
              <div style={{ gridColumn: "1/-1" }}><Input label="Shop Name" value={form.shopName} onChange={e => setForm(f => ({ ...f, shopName: e.target.value }))} placeholder="Your shop name" /></div>
              ...
              <Input label="Mobile 1" value={form.mobile1} onChange={e => setForm(f => ({ ...f, mobile1: e.target.value }))} placeholder="+91 99999 99999" />
              <Input label="Mobile 2" value={form.mobile2} onChange={e => setForm(f => ({ ...f, mobile2: e.target.value }))} placeholder="+91 99999 99999" />
              <div style={{ gridColumn: "1/-1" }}><Input label="Email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="email@example.com" /></div>
              <Input label="GSTIN" value={form.gstin} onChange={e => setForm(f => ({ ...f, gstin: e.target.value }))} placeholder="33XXXXX1234X1ZX" />
              <Input label="Website" value={form.website} onChange={e => setForm(f => ({ ...f, website: e.target.value }))} placeholder="www.yourshop.com" />
            </div>
          </>
        )}
        {tab === "bank" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
              <Input label="Bank Name" value={form.bankName} onChange={e => setForm(f => ({ ...f, bankName: e.target.value }))} placeholder="Bank name" />
              <Input label="Account Number" value={form.accountNo} onChange={e => setForm(f => ({ ...f, accountNo: e.target.value }))} placeholder="Account number" />
              <Input label="IFSC Code" value={form.ifsc} onChange={e => setForm(f => ({ ...f, ifsc: e.target.value }))} placeholder="IFSC code" />
              <div style={{ gridColumn: "1/-1" }}><Input label="UPI ID" value={form.upi} onChange={e => setForm(f => ({ ...f, upi: e.target.value }))} placeholder="yourname@upi" /></div>
            </div>
          </>
        )}
        {tab === "gst" && (
        ...
        {tab === "backup" && (
          <div>
            <h3 style={{ margin: "0 0 8px", color: BRAND.dark }}>Permanent Data Storage</h3>
            <p style={{ fontSize: 13, color: BRAND.gray, marginTop: 0, marginBottom: 20 }}>
              By default your data is saved in this browser only. To store it permanently in a
              real database (so it survives even if the browser is cleared, and can be shared
              across devices), deploy the included backend server and paste its address below.
              See <b>server/README.md</b> in your project for the free setup steps.
            </p>
            <Input label="Backend API URL" value={apiBase} onChange={e => setApiBase(e.target.value)} placeholder="https://your-app-name.onrender.com" />
            <div style={{ display: "flex", gap: 10, marginTop: 4, marginBottom: 16 }}>
              <button onClick={saveApiBase} style={{ padding: "10px 18px", background: BRAND.blue, color: "#fff", border: "none", borderRadius: 10, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>
                {apiSaved ? "Saved!" : "Save"}
              </button>
              <button onClick={testConnection} style={{ padding: "10px 18px", background: "#fff", color: BRAND.dark, border: `1.5px solid ${BRAND.border}`, borderRadius: 10, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>
                Test Connection
              </button>
              {apiStatus === "checking" && <span style={{ alignSelf: "center", fontSize: 13, color: BRAND.gray }}>Checking…</span>}
              {apiStatus === "ok" && <span style={{ alignSelf: "center", fontSize: 13, color: BRAND.green, fontWeight: 700 }}>✓ Connected</span>}
              {apiStatus === "fail" && <span style={{ alignSelf: "center", fontSize: 13, color: "#dc2626", fontWeight: 700 }}>✗ Could not reach server</span>}
            </div>
            {!apiBase && (
              <div style={{ background: BRAND.blueBg, borderRadius: 12, padding: 16, border: "1px solid #bfdbfe", fontSize: 13, color: BRAND.dark }}>
                No backend connected yet — your data is currently saved in this browser only.
              </div>
            )}
          </div>
        )}
...
function getApiBase() {
  try {
    return localStorage.getItem("billing_api_base") || "";
  } catch {
    return "";
  }
}
...
function usePersistentState(key, initialValue) {
  ...
  // On mount, if a backend URL is configured, fetch the latest saved value from MySQL.
  useEffect(() => {
    const apiBase = getApiBase();
    if (!apiBase) return;
    let cancelled = false;
    fetch(`${apiBase}/api/data/${key}`)
      .then(r => (r.ok ? r.json() : null))
      .then(data => {
        if (!cancelled && data && data.value !== undefined && data.value !== null) {
          setState(data.value);
        }
      })
      .catch(() => {
        // offline or backend unreachable — keep using local copy
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    if (apiBase) {
      fetch(`${apiBase}/api/data/${key}`, {
        ...
        // offline or backend unreachable — local copy still saved above
      });
    }
  }, [key, state]);
...
export default function App() {
  const [loggedIn, setLoggedIn] = usePersistentState("billing_loggedIn", false);
  const [page, setPage] = useState("dashboard");
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [products, setProducts] = usePersistentState("billing_products", initialProducts);
  const [customers, setCustomers] = usePersistentState("billing_customers", initialCustomers);
  const [sales, setSales] = usePersistentState("billing_sales", initialSales);
  const [expenses, setExpenses] = usePersistentState("billing_expenses", initialExpenses);
  const [logoUrl, setLogoUrl] = usePersistentState("billing_logoUrl", null);
  const [settings, setSettings] = usePersistentState("billing_settings", {
    ...
    <div style={{ display: "flex", height: "100vh", background: BRAND.grayLight, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 24 }}>
          ...
          {page === "settings" && <SettingsPage logoUrl={logoUrl} setLogoUrl={setLogoUrl} settings={settings} setSettings={setSettings} />}
        </div>
      </div>
    </div>
  );
